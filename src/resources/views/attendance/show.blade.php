...
<div class="attendance-detail__content">
    ...
    @php
        ...
        $req = $attendance->stampCorrectionRequest;
                ...
                <tr class="detail-table__row">
                    ...
                    <td class="detail-table__item">
                        <div class="time-input-group">
                            <input type="text" class="input-time-large" name="attendance[0][start_time]" 
                                value="{{ old('attendance.0.start_time', $attendance->start_time ? \Carbon\Carbon::parse($attendance->start_time)->format('H:i') : '') }}" @if($isWaiting || $isApproved) readonly @endif>
                            <span class="time-separator">～</span>
                            <input type="text" class="input-time-large" name="attendance[0][end_time]" 
                                value="{{ old('attendance.0.end_time', $attendance->end_time ? \Carbon\Carbon::parse($attendance->end_time)->format('H:i') : '') }}" @if($isWaiting || $isApproved) readonly @endif>
                        </div>
                        @if ($errors->has('attendance.0.start_time') || $errors->has('attendance.0.end_time'))
                            <div class="error-msg">出勤・退勤時間が不適切な値です</div>
                        @endif
                    </td>
                </tr>
                    ...
                    <td class="detail-table__item">
                        <div class="time-input-group">
                            <input type="text" class="input-time-large" name="attendance[2][start_time]"
                                value="{{ old('attendance.2.start_time', $rest2 ? \Carbon\Carbon::parse($rest2->start_time)->format('H:i') : '') }}" @if($isWaiting || $isApproved) readonly @endif>
                            <span class="time-separator">～</span>
                            <input type="text" class="input-time-large" name="attendance[2][end_time]"
                                value="{{ old('attendance.2.end_time', $rest2 ? \Carbon\Carbon::parse($rest2->end_time)->format('H:i') : '') }}" @if($isWaiting || $isApproved) readonly @endif>
                        </div>
